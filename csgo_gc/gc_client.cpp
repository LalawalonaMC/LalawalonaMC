...
ClientGC::ClientGC(uint64_t steamId)
    ...
    , m_inventory{ steamId }
{
    g_pActiveClient = this; // ADD THIS LINE HERE
...
ClientGC::~ClientGC()
{
    if (g_pActiveClient == this) {
        g_pActiveClient = nullptr; 
    }
    StopThread();
...
void ClientGC::TriggerFakeAccept()
{
    // 1. Create the Protobuf message
    CMsgGCCStrike15_v2_MatchmakingGC2ClientReadyCheck message;

    // 2. Set the fake data to trigger the UI
    message.set_match_id(1337);
    message.set_allow_overlap(true);
    message.set_status(1); // 1 = Match Found (Triggers the green button)

    // 3. Send it to the game client
    // Type 9108 is the ID for ReadyCheck
    SendMessageToGame(false, k_EMsgGCCStrike15_v2_MatchmakingGC2ClientReadyCheck, message);

    Platform::Print("DEBUG: Sent Fake Matchmaking Ready Check (Accept Button)\n");
}
